  ...
  if (item.price.type === "Fixed" || item.price.type === "Negotiable") {
    return (
      <>
        ...
        {item.price.amount.toLocaleString("en-KE")}
      </>
    );
  } else {
    return "FREE";
        ...
        <div className={`flex flex-col md:w-1/2 m-4 relative rounded-xl`}>
          ...
          <div className="my-4 flex justify-between">
            ...
            <button
              ...
              className={`font-[500] flex items-center text-white px-4 py-2 rounded-xl ${
                chatLoading || (item.status === "sold" && !item.selfOwned)
        ...
        <div className="flex flex-col md:w-1/2 m-4">
          ...
          <table className="w-full border mt-4">
            ...
            <tbody>
              ...
              <TableRow
                label="Delivery Area"
                value={deliveryType[item.delivery.type].area}
              />
              <TableRow
                label="Delivery Type"
                value={deliveryType[item.delivery.type].type}
              />
              {item.delivery.type !== "Door Pickup" && (
                <TableRow
                  label="Delivery Cost"
                  value={renderDeliveryCost(item.delivery.cost)}
                />
              )}
              <TableRow
                label="Owned By"
                value={
                  <Link className="flex gap-2" href={`/lists/${item.seller}`}>
                    <span
                      className={`font-[600] text-gray-800 flex items-center gap-1 hover:text-red-500 cursor-pointer`}
                    >
                      {item.seller_name}
