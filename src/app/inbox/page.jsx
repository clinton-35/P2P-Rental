...
async function getChatList() {
  try {
    const res = await fetch(
      process.env.NEXT_PUBLIC_URL + "/api/chats/getList",
      {
        method: "GET",
    ...
    return (
      <div className="max-w-screen-xl mx-auto mt-[80px]">
        <div className="m-4 p-6 rounded-2xl bg-white/60 backdrop-blur-xl shadow-xl flex flex-col items-center justify-center h-[calc(80vh-80px)] animate-fadeIn">
          ...
          <div className="text-lg font-bold mt-4 text-gray-700">
            Your inbox is empty
          </div>
          <p className="text-sm text-gray-500 mt-2">
            Chats with sellers will appear here.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="max-w-screen-xl mx-auto mt-[80px] flex flex-col gap-4 animate-fadeSlideUp">
      <div className="px-4 mt-2">
        <div className="text-3xl font-extrabold bg-gradient-to-r from-black to-gray-600 bg-clip-text text-transparent tracking-tight">
          My Messages
        </div>
      </div>
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
              <div className="font-semibold text-gray-900 text-lg">
                {chat.users.find((u) => u.name !== session.user.name)?.name ||
                  "Unknown User"}
              </div>
              ...
              <div className="text-sm text-gray-500 truncate max-w-[220px]">
                {chat.messages?.length > 0 ? (
                  <>
                    {chat.messages[0].sender?._id === chat.me[0]?._id
                      ? "You: "
                      : ""}
          ...
          <div className="text-xs text-gray-500 flex items-center gap-2">
            ...
            >
              <path
                fill="currentColor"
                d="M216 48H40a16 16 0 0 0-16 16v160a15.84 15.84 0 0 0 9.25 14.5A16.05 16.05 0 0 0 40 240a15.89 15.89 0 0 0 10.25-3.78a.69.69 0 0 0 .13-.11L82.5 208H216a16 16 0 0 0 16-16V64a16 16 0 0 0-16-16Z"
              />
            </svg>
            {chat.messages?.length > 0
